    ...
    <div class="login-body" :class="{ weChatBody: showWeChatBody }">
      ...
      <div v-show="!succeeded">
        ...
        <h1 class="login-h1" v-show="showLanguage == 'en' || !showLanguage">
          Please Use DAYLIGHT PMS User Credential to Login Hotel
          Toolkits
        </h1>
        <h1 class="login-h1" v-show="showLanguage == 'zh' || !showLanguage">
          请使用 DAYLIGHT PMS 用户名和密码登录 Hotel Toolkits
        </h1>
        <!-- 登录报错提示 -->
        <div
          ...
          <div v-show="!showFeet" class="distance-button">
            ...
            <div v-show="showAll">
              <!-- <router-link
                ...
                replace
                to="/login"
                >Log In Using Default Account</router-link
              > -->
              ...
              <div v-show="showshijiEmail">
                <router-link class="a-style" replace to="/shijiEmailLogin">
                  <span v-show="showLanguage == 'en' || !showLanguage"
                    >Login With ShiJi Email Account</span
                  ><span v-show="!showLanguage"> | </span
                  ><span v-show="showLanguage == 'zh' || !showLanguage"
                    >使用石基邮箱登录</span
                  >
...
export default {
  ...
  created() {
    console.log(this.processPage, "this.processPage-->SEP");
    // 判断页面是否去了处理逻辑的页面
    if (!this.processPage) {
      this.$router.replace("/");
      return;
    ...
    //判断本地存储中有没有承租人
    if (
      localStorage.getItem(SEP_IDENTITY_TITLE + "-" + "sepTenantId") != null &&
      !this.showTenant
    ) {
      this.form.TenantId = localStorage.getItem(
        SEP_IDENTITY_TITLE + "-" + "sepTenantId"
      );
  ...
  methods: {
    ...
    removeTenant() {
      // 点击清除 sepTenantId
      localStorage.removeItem(SEP_IDENTITY_TITLE + "-" + "sepTenantId");
      this.form.TenantId = "";
      ...
      if (
        ...
        this.$httpServiceSEP({
          ...
          .then((res) => {
            ...
            // 储存SEP登录的登录方式
            this.$cookies.set(`${this.realmNameUrl}_sep`, "sepLogin");
            // 删除其余登录方式
